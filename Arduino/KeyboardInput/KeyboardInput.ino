...
int characters[4] = {97, 100, 32, 129};           // Charaktere nach ASCII (a, d, Leertaste, shift)
boolean isDown[4] = {false, false, false, false}; // Array mit Status aller Tasten
boolean wasDown[4];                               // Array mit vorherigen Status

// Initialisierungs-Funktion, wird bei Arduino-start 1x durchlaufen
void setup() {
  Keyboard.begin();                   // Tastatur Emulierung wird gestartet
  pinMode(buttonA, INPUT_PULLUP);     // Pins als Tasten-Input, interner Widerstand wird verwendet
...
void loop() {
  ...
  isDown[0] = !digitalRead(buttonA); // Status von Pins wird überprüft und gespeichert
  isDown[1] = !digitalRead(buttonD);
  isDown[2] = !digitalRead(buttonSpace);
  isDown[3] = !digitalRead(buttonShift);
  for (int k = 0; k < 4; k++) {processKey(k);} // Jede Taste wird verarbeitet
